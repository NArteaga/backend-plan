...
module.exports = function setupSeguimientoController (services) {
  ...
  async function findAll (req, res) {
    try {
      const respuesta = await SeguimientoService.findAll(req.query);
  ...
  async function mostrar (req, res) {
    try {
      ...
      const respuesta = await SeguimientoService.findOne({ id: req.params.id });
      return res.status(200).send(new Respuesta('OK', Finalizado.OK, respuesta));
    } catch (error) {
      return res.status(error.httpCode || HttpCodes.userError).json(new Respuesta(error.message, Finalizado.FAIL));
    }
  ...
  async function crear (req, res) {
    try {
      ...
      const respuesta = await SeguimientoService.createOrUpdate(datos);
      return res.status(200).send(new Respuesta('OK', Finalizado.OK, respuesta));
    } catch (error) {
      return res.status(error.httpCode || HttpCodes.userError).json(new Respuesta(error.message, Finalizado.FAIL));
    }
  ...
  async function eliminar (req, res) {
    try {
      const respuesta = await SeguimientoService.eliminar(req.params.id);
      return res.status(200).send(new Respuesta('OK', Finalizado.OK, respuesta));
    } catch (error) {
      return res.status(error.httpCode || HttpCodes.userError).json(new Respuesta(error.message, Finalizado.FAIL));
    }
